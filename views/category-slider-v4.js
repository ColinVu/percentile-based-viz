...
function renderCategoryMetricListV4() {
  ...
  if (!listEl) return;
  // Exclude FIPS code metrics (handle variants: "FIPS_Code", "F I P S Code", "FIPS Code")
  ...
  requestAnimationFrame(() => {
    ...
    selectCategoryIndexV4(defaultIdx, { render: true });

    // Label click -> snap
    labelNodes.forEach((node, idx) => {
      node.addEventListener('click', () => selectCategoryIndexV4(idx, { render: true }));
    });
    ...
    const onPointerMove = (evt) => {
      ...
      selectCategoryIndexV4(nearestIdx, { render: true });
    };
    const onPointerUp = () => {
      dragging = false;
  ...
  function selectCategoryIndexV4(index, opts = {}) {
...
// Export functions
window.renderCategoryMetricListV4 = renderCategoryMetricListV4;