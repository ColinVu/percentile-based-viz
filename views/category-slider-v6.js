...
function renderCategoryMetricListV6() {
  ...
  requestAnimationFrame(() => {
    ...
    selectCategoryIndexV6(defaultIdx, { render: true });

    labelNodes.forEach((node, idx) => {
      node.addEventListener('click', () => selectCategoryIndexV6(idx, { render: true }));
    });
    ...
    const onPointerMove = (evt) => {
      ...
      selectCategoryIndexV6(nearestIdx, { render: true });
    };
    const onPointerUp = () => {
      dragging = false;
  ...
  function selectCategoryIndexV6(index, opts = {}) {
    ...
    if (render && typeof window.renderBeeswarmCategoryEncoded === 'function') {
      window.renderBeeswarmCategoryEncoded(metricKey);
    }
  }
}

window.renderCategoryMetricListV6 = renderCategoryMetricListV6;
